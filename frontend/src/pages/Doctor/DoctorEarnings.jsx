  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Track your revenue and financial growth.</p>
      </div>
      ...
      {data.totalEarnings === 0 ? (
        ...
        >
          ...
          <div>
            <h3 className="text-xl font-bold mb-2 text-center text-foreground">Estimated Monthly Growth Potential</h3>
            <p className="text-sm text-muted-foreground text-center mb-8">Assuming a standard consultation fee of ₹500/visit. This is for educational illustration only.</p>
          ...
          {/* Growth Tips */}
          <div>
            <h3 className="text-xl font-bold mb-6 text-foreground">Growth Tips</h3>
          ...
          <div className="text-center pb-8 mt-12">
            <Link to="/doctor/requests" className="inline-flex items-center justify-center px-10 py-4 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-bold text-lg shadow-lg shadow-primary/20 transition hover:-translate-y-1">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="glass-panel p-6 rounded-2xl border border-border relative overflow-hidden">
              ...
              <h3 className="text-muted-foreground font-medium text-sm mb-2 relative z-10">Total Earnings</h3>
              <p className="text-3xl font-bold text-primary relative z-10">₹{data.totalEarnings}</p>
            </div>
            <div className="glass-panel p-6 rounded-2xl border border-border">
              <h3 className="text-muted-foreground font-medium text-sm mb-2">This Month</h3>
              <p className="text-3xl font-bold text-foreground">₹{data.thisMonthEarnings}</p>
            </div>
            <div className="glass-panel p-6 rounded-2xl border border-border">
              <h3 className="text-muted-foreground font-medium text-sm mb-2">This Week</h3>
              <p className="text-3xl font-bold text-foreground">₹{data.thisWeekEarnings}</p>
            </div>
            <div className="glass-panel p-6 rounded-2xl border border-border">
              <h3 className="text-muted-foreground font-medium text-sm mb-2">Completed Consultations</h3>
          ...
          {/* Chart Section */}
          <div className="glass-panel p-6 rounded-2xl border border-border">
            <h3 className="text-xl font-bold mb-6 text-foreground">Earnings Trend</h3>
            <div className="h-64 w-full flex items-end gap-2 sm:gap-4 justify-between pt-8 border-b border-border relative">
              ...
              {chartData.map((data, index) => {
                ...
                return (
                  <div key={index} className="flex-1 flex flex-col items-center group relative z-10">
                    {/* Tooltip */}
                    <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-popover border border-border text-foreground px-3 py-1 rounded-lg text-sm shadow-lg pointer-events-none whitespace-nowrap z-20">
                    ...
                    ></div>
                    {/* X-Axis Label */}
                    <span className="text-xs text-muted-foreground mt-4 font-medium">{data.name}</span>
                  </div>
                );
              })}
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {data.recentTransactions.map((tx) => (
                      <tr key={tx._id} className="border-b border-border last:border-0 hover:bg-popover transition-colors">
                        <td className="py-4 font-medium text-foreground">{tx.patientName}</td>
                        ...
                        <td className="py-4 text-right font-bold text-foreground">₹{tx.fee}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
