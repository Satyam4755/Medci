    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      >
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          <div className="grid grid-cols-12 gap-4 text-sm font-bold text-muted-foreground uppercase tracking-wider mb-2">
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      {!hasAvailability ? (
        ...
        >
          ...
          {/* Benefits Section */}
          <div>
            <h3 className="text-xl font-bold mb-6 text-center text-foreground">Why Set Your Schedule?</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <div key={idx} className="bg-card p-6 rounded-2xl border border-border text-center group hover:border-primary transition">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="glass-panel p-6 rounded-2xl border border-border">
              <h3 className="text-muted-foreground font-medium text-sm mb-2">Active Days</h3>
              <p className="text-3xl font-bold text-primary">{activeDays} <span className="text-lg text-muted-foreground font-normal">/ week</span></p>
            </div>
            <div className="glass-panel p-6 rounded-2xl border border-border">
              <h3 className="text-muted-foreground font-medium text-sm mb-2">Total Hours</h3>
              <p className="text-3xl font-bold text-foreground">~{Math.round(totalHours)} <span className="text-lg text-muted-foreground font-normal">hrs</span></p>
            </div>
            <div className="glass-panel p-6 rounded-2xl border border-border">
          ...
          <div className="glass-panel rounded-2xl border border-border overflow-hidden">
            <div className="bg-popover p-6 border-b border-border">
            ...
            <div className="divide-y divide-[var(--color-theme-border)]">
              {DAYS_OF_WEEK.map((day) => {
                ...
                return (
                  <div key={day} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-card/50 transition-colors gap-4">
                    <div className="flex items-center gap-4">
                      <div className={`w-3 h-3 rounded-full ${dayData ? 'bg-green-500' : 'bg-neutral-600'}`}></div>
                      <span className={`font-medium text-lg ${dayData ? 'text-foreground' : 'text-muted-foreground'}`}>{day}</span>
                    </div>
                    <div>
                      {dayData ? (
                        <div className="inline-flex items-center gap-3 bg-popover px-4 py-2 rounded-lg border border-border">
                        ...
                        <span className="px-4 py-2 text-muted-foreground italic">Unavailable</span>
                      )}
                    </div>
                  </div>
                )
